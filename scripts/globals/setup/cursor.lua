...
cursor.setDefault({
    ...
    refresh = function(evtData)
        ...
        cursor._imgPointer:texture(texture)
        cursor._imgPointer:alpha(alpha + cn)
        cursor._imgPointer:size(width, height)
        cursor._imgPointer:relation(UI_ALIGN_LEFT_TOP, UIGame, UI_ALIGN_LEFT_BOTTOM, japi.UIDisAdaptive(rx), ry)
    end
})
...
cursor.setQuote(ability.targetType.unit, {
    ...
    refresh = function(evtData)
        ...
        cursor._imgPointer:texture(texture)
        cursor._imgPointer:alpha(alpha)
        cursor._imgPointer:size(width, height)
        cursor._imgPointer:relation(UI_ALIGN_CENTER, UIGame, UI_ALIGN_LEFT_BOTTOM, japi.UIDisAdaptive(rx), ry)
    end,
...
cursor.setQuote(ability.targetType.circle, {
    ...
    refresh = function(evtData)
        ...
        cursor._imgArea:rgba(255, 255, 255, circleParams.alpha)
        cursor._imgArea:texture(texture)
        cursor._imgArea:size(curSize * 2, curSize * 2)
        cursor._imgArea:position(tx, ty)
        cursor._imgArea:show(true)
    end,
    ---@param evtData evtOnMouseLeftClickData
    leftClick = function(evtData)
        local data = cursor.currentData()
...
cursor.setQuote(ability.targetType.square, {
    ...
    refresh = function(evtData)
        ...
        cursor._imgArea:rgba(255, 255, 255, csTexture.square.alpha)
        cursor._imgArea:texture(texture)
        cursor._imgArea:size(curWidth, curHeight)
        cursor._imgArea:position(tx, ty)
        cursor._imgArea:show(true)
    end,
    ---@param evtData evtOnMouseLeftClickData
...
cursor.setQuote("follow", {
    start = function()
        ...
        local rx, ry = japi.MouseRX(), japi.MouseRY()
        cursor._imgFollow:texture(texture)
              :size(width, height)
    ...
    ---@param evtData evtOnMouseMoveData
    refresh = function(evtData)
        local w, h = cursor._imgFollow._width, cursor._imgFollow._height
        local rx, ry = evtData.rx, evtData.ry
        ...
        cursor._imgFollow:relation(UI_ALIGN_CENTER, UIGame, UI_ALIGN_LEFT_BOTTOM, rx, ry)
    end,
